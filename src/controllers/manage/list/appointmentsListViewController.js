...
angular.module('bahmni.appointments').controller('AppointmentsListViewController', [
  '$scope',
  '$state',
  '$rootScope',
  '$translate',
  ...
  function (
    ...
    $scope.priorityOptionsList =
      appService.getAppDescriptor().getConfigValue('priorityOptionsList') || [];
    $scope.allowedActions = appService.getAppDescriptor().getConfigValue('allowedActions') || [];
    $scope.allowedActionsByStatus =
      appService.getAppDescriptor().getConfigValue('allowedActionsByStatus') || {};
    $scope.colorsForListView =
      appService.getAppDescriptor().getConfigValue('colorsForListView') || {};
    var maxAppointmentProviders =
      appService.getAppDescriptor().getConfigValue('maxAppointmentProviders') || 1;
    var allowVirtualConsultation = appService
      .getAppDescriptor()
      .getConfigValue('allowVirtualConsultation');

    $scope.enableResetAppointmentStatuses = appService
      .getAppDescriptor()
      .getConfigValue('enableResetAppointmentStatuses');
    $scope.isAppointmentRequestEnabled = appService
      .getAppDescriptor()
      .getConfigValue('enableAppointmentRequests');
    ...
    $scope.resetAppointmentStatusPrivilege =
      Bahmni.Appointments.Constants.privilegeResetAppointmentStatus;
    $scope.shouldShowAppointmentsListPatientLink =
      !!Bahmni.Common.Constants.appointmentsListPatientLink;
    $scope.searchedPatient = false;
    $scope.enableColumnForTab = function (tabName) {
      if ($scope.getCurrentTabName() === tabName) return true;
      return false;
    };
    var autoRefreshIntervalInSeconds = parseInt(
      appService.getAppDescriptor().getConfigValue('autoRefreshIntervalInSeconds'),
    );
    ...
    var updateTableHeader = function () {
      $scope.tableInfo = [
        ...
        {
          heading: 'APPOINTMENT_CREATION_DATE',
          sortInfo: 'dateCreated',
          class: true,
          enable: !$scope.enableColumnsForAppointments,
        },
        {
        ...
        { heading: 'APPOINTMENT_SERVICE', sortInfo: 'service.name', class: true, enable: true },
        {
          ...
          sortInfo: 'serviceType.name',
          class: true,
          enable: $scope.enableServiceTypes,
        },
        { heading: 'APPOINTMENT_STATUS', sortInfo: 'status', enable: true },
        {
          heading: 'APPOINTMENT_WALK_IN',
          sortInfo: 'appointmentKind',
          enable: $scope.enableColumnsForAppointments,
        },
        {
          heading: 'APPOINTMENT_SERVICE_LOCATION_KEY',
          sortInfo: 'location.name',
          class: true,
          enable: true,
        },
        {
          heading: 'APPOINTMENT_ADDITIONAL_INFO',
          sortInfo: 'additionalInfo',
          class: true,
          enable: true,
        },
        ...
        return appointmentsService
          .search(
            $location.search()['patient']
              ? { patientUuid: $location.search()['patient'] }
              : APPOINTMENT_STATUS_WAITLIST,
          )
          .then((response) => updateAppointments(response));
      }
    };
    ...
    var setAppointmentsInPatientSearch = function (patientUuid) {
      appointmentsService.search({ patientUuid: patientUuid }).then(function (response) {
        var appointmentsInDESCOrderBasedOnStartDateTime = _.sortBy(
          response.data,
          'startDateTime',
        ).reverse();
    ...
    $scope.translateAppointmentStatus = function (appointmentStatus) {
      if (appointmentStatus === 'WaitList') return $translate.instant('APPOINTMENT_WAITLIST');
    ...
    var autoRefresh = (function () {
      ...
      var autoRefreshIntervalInMilliSeconds =
    ...
    $scope.hasNoAppointments = function () {
      //   console.log(
      //     'patient',
      //     $state.params.patient.label,
      //     $state.params.patient.label.includes('undefined'),
      //   );
      if ($state.params.patient && $state.params.patient.label === 'undefined (undefined)') {
    ...
    $scope.generateAppointmentsListPatientLink = function (appointment) {
      return appService
        .getAppDescriptor()
        .formatUrl(Bahmni.Common.Constants.appointmentsListPatientLink, {
    ...
    $scope.$watch(
      ...
      function (newValue, oldValue) {
        if (newValue !== oldValue && !isNullOrEmpty(oldValue)) {
          $scope.filteredAppointments = appointmentsFilter(
            $scope.appointments || $state.params.appointmentsData,
            $stateParams.filterParams,
          );
        }
      },
    ...
    $scope.sortAppointmentsBy = function (sortColumn) {
      ...
      if (sortColumn === 'provider.name') {
        emptyObjects = $scope.filteredAppointments.filter(function (fa) {
          return (
            _.every(fa.providers, {
              response: Bahmni.Appointments.Constants.providerResponses.CANCELLED,
            }) || _.isEmpty(fa.providers)
          );
        });
      }
    ...
    $scope.printPage = function () {
      var printTemplateUrl =
        appService.getAppDescriptor().getConfigValue('printListViewTemplateUrl') ||
        'views/manage/list/defaultListPrint.html';
    ...
    var resetStatus = function (closeConfirmBox) {
      return appointmentsService
        .changeStatus($scope.selectedAppointment.uuid, 'Scheduled', null)
    ...
    $scope.isSelectedAppointmentAwaitingForCurrentProvider = function () {
      ...
      const currentProviderInAppointment = findCurrentProviderInAppointment(
        $scope.selectedAppointment,
      );
      return (
        !_.isUndefined(currentProviderInAppointment) &&
        currentProviderInAppointment.response ===
    ...
    function acceptAppointmentInvite(closeConfirmBox) {
      ...
      return appointmentsService
        .changeProviderResponse(
          $scope.selectedAppointment.uuid,
          currentProviderInAppointment.uuid,
          Bahmni.Appointments.Constants.providerResponses.ACCEPTED,
        )
        .then(function () {
          currentProviderInAppointment.response =
            Bahmni.Appointments.Constants.providerResponses.ACCEPTED;
    ...
    var changeStatus = function (toStatus, onDate, closeConfirmBox, applyForAll) {
      ...
      return appointmentsService
        .changeStatus($scope.selectedAppointment.uuid, toStatus, onDate, applyForAll)
        .then(function (response) {
          ngDialog.close();
          $scope.selectedAppointment.status = response.data.status;
          closeConfirmBox();
    ...
    var showPopUp = function (popUpScope) {
      ...
      var actions = [
        { name: 'yes', display: 'YES_KEY' },
        { name: 'no', display: 'NO_KEY' },
      ];
      if (popUpScope.yes_all !== undefined) {
        actions = [
          { name: 'yes', display: 'RECURRENCE_THIS_APPOINTMENT' },
    ...
    $scope.isValidActionAndIsUserAllowedToPerformEdit = function (action) {
      if (!_.isUndefined($scope.selectedAppointment)) {
        ...
        return !appointmentCommonService.isUserAllowedToPerformEdit(
          appointmentProvider,
    ...
    var isValidAction = function (action) {
      ...
      var allowedActions = $scope.allowedActionsByStatus.hasOwnProperty(status)
        ? $scope.allowedActionsByStatus[status]
        : [];
    ...
    var isCurrentUserHavePrivilege = function (privilege) {
      return !_.isUndefined(
        _.find($rootScope.currentUser.privileges, function (userPrivilege) {
    ...
    $scope.isUserAllowedToPerform = function () {
      if (isCurrentUserHavePrivilege($scope.manageAppointmentPrivilege)) {
        ...
        if ($scope.selectedAppointment) {
          return (
            _.isNull($scope.selectedAppointment.provider) ||
    ...
    $scope.isEditAllowed = function () {
      if (!_.isUndefined($scope.selectedAppointment)) {
        var appointmentProvider = $scope.selectedAppointment.providers;
        return maxAppointmentProviders > 1
          ? true
          : appointmentCommonService.isUserAllowedToPerformEdit(
              appointmentProvider,
              currentUserPrivileges,
              currentProviderUuId,
            );
    ...
    $scope.isTeleconsultationAllowed = function () {
      if (!_.isUndefined($scope.selectedAppointment)) {
        return (
          $scope.selectedAppointment.appointmentKind === 'Virtual' &&
          $scope.selectedAppointment.status ==
            window.Bahmni.Appointments.Constants.appointmentStatuses.Scheduled
        );
      }
      return false;
    };
    ...
    $scope.isResetAppointmentStatusFeatureEnabled = function () {
      return !(
        _.isNull($scope.enableResetAppointmentStatuses) ||
    ...
    var isSelectedAppointmentStatusAllowedToReset = function () {
      return (
        _.isArray($scope.enableResetAppointmentStatuses) &&
