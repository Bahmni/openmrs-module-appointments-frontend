...
angular.module('bahmni.appointments').directive('patientSearch', [
  'patientService',
  'appointmentsService',
  'spinner',
  '$state',
        ...
        pre: function ($scope) {
          $scope.search = function () {
            return spinner.forPromise(
              patientService.search($scope.patient).then(function (response) {
          ...
          $scope.responseMap = function (data) {
            return _.map(data, function (patientInfo) {
              ...
              patientInfo.label =
                patientInfo.givenName + familyName + ' ' + '(' + patientInfo.identifier + ')';
              return patientInfo;
            });
          };
          ...
          $scope.onSelectPatient = function (data) {
            ...
            spinner.forPromise(
              appointmentsService
                .search({ patientUuid: data.uuid })
                .then(function (oldAppointments) {
                  var appointmentInDESCOrderBasedOnStartDateTime = _.sortBy(
                    oldAppointments.data,
                    'startDateTime',
                  ).reverse();
                  $scope.onSearch(appointmentInDESCOrderBasedOnStartDateTime);
                  $scope.filteredAppointments = appointmentInDESCOrderBasedOnStartDateTime;
                }),
            );
          };
