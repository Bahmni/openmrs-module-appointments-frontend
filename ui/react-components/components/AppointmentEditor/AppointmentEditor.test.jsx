...
describe('Appointment Editor', () => {
    ...
    it('should call conflicts api when checkAndSave is clicked', async () => {
        const {getByText} = renderWithReactIntl(<AppointmentEditor/>);
        const appointmentConflictsSpy = jest.spyOn(apiService, 'getAppointmentConflicts');
        const checkAndSaveButton = getByText('Check and Save');
        fireEvent.click(checkAndSaveButton);
        expect(appointmentConflictsSpy).toHaveBeenCalled();
    });
});
